...
import io.github.pascalgrimaud.sample.web.rest.AlphaKafkaResource.SafeKafkaConsumer;
...
class AlphaKafkaResourceIT {

    private static boolean started = false;
    private static KafkaContainer kafkaContainer;

    private AlphaKafkaResource kafkaResource;
    ...
    void setup() {
        ...
        kafkaResource = new AlphaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/alpha-kafka/publish/topic-produce?message=value-produce"))
    ...
    void createsConsumer() throws Exception {
        restMockMvc.perform(post("/api/alpha-kafka/consumers?name=consumer-create&topic=topic-create&topic=topic-create2&group.id=group-create"))
            .andExpect(status().isOk());

        Field consumersField = ReflectionUtils.findField(AlphaKafkaResource.class, "consumers");
    ...
    void consumesMessages() throws Exception {
        ...
        restMockMvc.perform(get("/api/alpha-kafka/consumers/consumer-consume/records?durationMs=5000"))
            .andExpect(status().isOk())
